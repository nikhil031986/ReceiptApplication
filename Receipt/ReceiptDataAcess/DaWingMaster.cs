...
namespace ReceiptDataAcess
{
    public static class DaWingMaster
    {
        public static async Task<List<EnWingMaster>> GetWingMaster(int WingMasterId = 0)
        {
            ...
            try
            {
                cmd.CommandText = "SELECT * FROM Wing_Master" + (WingMasterId == 0 ? ";" : " WHERE Wing_Master_Id=@Wing_Master_Id;");
                if (WingMasterId > 0)
                {
        ...
        public static async Task<EnWingMaster> getWingMaster(int wingMasterId)
        {
            ...
            try
            {
                command.CommandText = "SELECT * FROM Wing_Master WHERE Wing_Master_Id=@Wing_Master_Id;";
                ...
                if (dtWingMaster != null && dtWingMaster.AsEnumerable().Count() > 0)
                {
                    ...
                    command.CommandText = "SELECT * FROM Wing_Details WHERE Wing_MasterId=@Wing_MasterId";
                    command.Parameters.Clear();
        ...
        public static async Task<List<EnWingDetails>> GetWingDetails(int wingMasterId=0)
        {
            ...
            try
            {
                cmd.CommandText = "SELECT * FROM Wing_Details" + (wingMasterId == 0 ? ";" : " WHERE Wing_MasterId=@Wing_MasterId;");
                if (wingMasterId > 0)
                {
                    cmd.Parameters.Clear();
        ...
        private static async Task<List<int>> InsertUpdateWingDetails(List<EnWingDetails> wingDetails, int wingMasterId)
        {
            ...
            foreach (EnWingDetails wingDetail in wingDetails)
            {
                ...
                try
                {
                    ...
                    {
                        strCommand = "Insert into Wing_Details (Wing_MasterId,FlatNo,Wing_Name) VALUES(@Wing_MasterId,@FlatNo,@Wing_Name); SELECT last_insert_rowid() FROM Wing_Details;";
                    }
                    else
                    {
                        isEdit = true;
                        strCommand = "Update Wing_Details set Wing_MasterId=@Wing_MasterId,FlatNo=@FlatNo,Wing_Name=@Wing_Name WHERE Wing_DetailsId=@Wing_DetailsId; SELECT @Wing_DetailsId;";
                    }
                    cmd.CommandText = strCommand;
                    cmd.CommandType = System.Data.CommandType.Text;
                    cmd.Parameters.Clear();
